...
export default function CreateOrganization() {
  ...
  async function handleSubmit(e: FormEvent) {
    ...
    setLoading(true);
    try {
      await api.post('/organizations', { name, org_type: orgType });
      navigate('/organization');
    } catch (err: unknown) {
      // 409 here means "you already belong to an organization" --
      // the backend enforces one org per user at the RPC level, not
      // just as a UI nicety. Surface the real message either way.
      ...
      <form onSubmit={handleSubmit} className="bg-slate-900 p-8 rounded-lg shadow-lg w-full max-w-sm space-y-4">
        <div>
          ...
          <p className="text-sm text-slate-400 mt-1">
            You'll be the owner. You can invite others once it's set up.
          </p>
        </div>
        <div>
